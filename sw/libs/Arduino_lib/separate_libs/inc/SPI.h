...
public:
  ...
  SPISettings() {
    init_AlwaysInline(5000000, MSBFIRST, SPI_MODE0);
  }
private:
...
public:
  ...
  inline static uint8_t transfer(uint8_t data) {
    ...
    SPI_SPILEN= (8<<16) | 8;	//set data length to be 8 bits, address and command lengths 8 bits 
    while (((SPI_STATUS >> 24) & 0xFF) >= 8);	//wait until tx buffer has available place
    SPI_SPICMD = data<<24;
    SPI_STATUS = 0x0101;    //initiate a write operation with select CS0
    //while((SPI_STATUS & 0x01)==0);	//wait until SPI is idle
