/*
    ...
    Filename:      ram_256x32_2p.v
    IP code :      S55NLLG2PH
    Version:       1.1.0
    CreateDate:    Nov 27, 2016

    Verilog Model for General 2-PORT SRAM
    SMIC 55nm LL Logic Process

    Configuration: -instname ram_256x32_2p -rows 128 -bits 32 -mux 2 
    Redundancy: Off
    Bit-Write: Off
*/
...
(CENA_latched!==1'b1);
  ...
  specify
    ...
    $period(posedge CLKA,1.089,CLKA_CYC_flag);
    $width(posedge CLKA,0.327,0,CLKA_H_flag);
    $width(negedge CLKA,0.327,0,CLKA_L_flag);

    $setuphold(posedge CLKB,posedge CENB,0.500,0.250,CENB_flag);
    $setuphold(posedge CLKB,negedge CENB,0.500,0.250,CENB_flag);
    $period(posedge CLKB,1.089,CLKB_CYC_flag);
    $width(posedge CLKB,0.327,0,CLKB_H_flag);
    $width(negedge CLKB,0.327,0,CLKB_L_flag);
