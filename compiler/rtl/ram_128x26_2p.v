/*
    ...
    Configuration: -instname ram_128x26_2p -rows 64 -bits 26 -mux 2 
    Redundancy: Off
    Bit-Write: Off
*/
...
(CENA_latched!==1'b1);
  ...
  specify
    ...
    $period(posedge CLKA,0.987,CLKA_CYC_flag);
    $width(posedge CLKA,0.296,0,CLKA_H_flag);
    $width(negedge CLKA,0.296,0,CLKA_L_flag);

    $setuphold(posedge CLKB,posedge CENB,0.500,0.250,CENB_flag);
    $setuphold(posedge CLKB,negedge CENB,0.500,0.250,CENB_flag);
    $period(posedge CLKB,0.987,CLKB_CYC_flag);
    $width(posedge CLKB,0.296,0,CLKB_H_flag);
    $width(negedge CLKB,0.296,0,CLKB_L_flag);
